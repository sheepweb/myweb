...
# 查看应用日志（将项目目录换为 /opt/cboard 或 /www/wwwroot/你的域名）
tail -f /opt/cboard/server.log
```
...
- 检查端口是否正确：`netstat -tlnp | grep 8000` 或 `ss -tlnp | grep 8000`
- 检查 Nginx 配置中的 `proxy_pass` 地址是否指向后端端口
...
```bash
# 进入项目目录（无宝塔：/opt/cboard，宝塔：/www/wwwroot/你的域名）
cd /opt/cboard
chmod 666 cboard.db
...
- 使用安装脚本重置管理员密码：进入项目目录执行 `sudo ./install.sh`，选择 **2（创建/重置管理员账号）**（仅宝塔）；或使用 `go run scripts/admin_tool.go <新密码>` 重置密码
- 账户被锁定：`go run scripts/unlock_user.go <用户名或邮箱>` 解锁（支持管理员和普通用户）
